...
export default function AdminClientLayout({ children }: { children: React.ReactNode }) {
    ...
    const handleLogout = async () => {
        // Panggil server action karena cookie admin_token berstatus httpOnly
        await logoutAdmin();

        // Pindah ke root dan paksa reload supaya sisi server ikut membaca state tanpa cookie
    ...
    const NavLinks = () => (
        <>
            ...
            <button
                ...
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all text-red-400 hover:text-red-300 hover:bg-red-950/30 text-left mt-4 border border-transparent hover:border-red-900/30 cursor-pointer"
            >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
